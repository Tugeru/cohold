...
**State helpers:** contract source of truth is the Rust crate `contracts/cohold/src/lib.rs` (Inspector reads it via `GET /api/contract-source`); `src/lib/contract-invariants.ts` keeps the displayed security invariants; plus `src/lib/stellar.ts`, `src/db/*`, `src/lib/db-seed.ts`, `src/lib/personas.ts`.
